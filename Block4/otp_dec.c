...
int main(int argc, char *argv[])
{
    ...
    //open ciphertext file and get contents
	...
	//Compare ciphertext and key to make sure key is big enough
    ...
    //concatenate indicator, ciphertext, key together for sending to server, use "." as delimiter
    ...
    //Send for decryption. Source: Piazza @547
    int acc = 0;
    ...
    //If get test byte back as "n", close program
    /*******
    int recByte;
    ...
    {
        printf("Error: otp_dec not allowed to access otp_enc_d port\n");
        exit(1);
    }
    ***************/
	// Send message to server
	//charsWritten = send(socketFD, textBuffer2, strlen(textBuffer2), 0); // Write to the server
    ...
    while(strstr(finalText, "$$") == NULL) {
        ...
        strcat(finalText, recBuffer);
        }

        if(strstr(finalText, "@@") != NULL)
        {
            printf("Error: otp_dec cannot connect to this daemon!\n");
            exit(1);
        }

        int terminalLocation = strstr(finalText, "$$") - finalText;
        finalText[terminalLocation] = '\0';
	...
	printf("%s\n", finalText);    //print final cipher text
